...
fn main() -> anyhow::Result<()> {
    ...
    if let Ok(content) = fs::read_to_string(file_path) {
        ...
        let use_statements = content.lines().filter(|line| line.trim().starts_with("use ")).count();
        let functions = content.matches("fn ").count();
        let structs = content.matches("struct ").count();
        let impl_blocks = content.matches("impl ").count();
        let comments = content.lines().filter(|line| line.trim().starts_with("//")).count();
        ...
        let long_lines: Vec<_> = content.lines()
            .enumerate()
    ...
    println!("⏱️  Indexer initialization: {:.3}s", init_time.as_secs_f64());
    ...
    println!("⏱️  Total symbol extraction: {:.3}s", total_extraction_time.as_secs_f64());
    println!("📊 Extracted {} symbols", symbols.len());
    
    if !symbols.is_empty() {
        println!("   Symbols per second: {:.1}", symbols.len() as f64 / total_extraction_time.as_secs_f64());
        println!("   Time per symbol: {:.1}ms", total_extraction_time.as_millis() as f64 / symbols.len() as f64);
    }
    ...
    for symbol in &symbols {
        *symbol_counts.entry(format!("{:?}", symbol.symbol_type)).or_insert(0) += 1;
    }
    ...
    println!("   Initialization: {:.3}s ({:.1}%)", init_time.as_secs_f64(), init_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
    println!("   File reading: {:.3}s ({:.1}%)", read_time.as_secs_f64(), read_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
    println!("   Symbol extraction: {:.3}s ({:.1}%)", total_extraction_time.as_secs_f64(), total_extraction_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
    ...
    println!("   Performance ratio: {:.1}x {} expected", performance_ratio, 
             if performance_ratio > 1.0 { "slower than" } else { "faster than" });
    
    if performance_ratio > 10.0 {
        println!("   🚨 SEVERE PERFORMANCE ISSUE DETECTED!");
        println!("   This file is taking {}x longer than expected.", performance_ratio as u32);
        println!("   Investigate: complex regex patterns, deep nesting, or tree-sitter parsing issues.");
    } else if performance_ratio > 3.0 {
        println!("   ⚠️  Performance issue detected ({}x slower than expected)", performance_ratio as u32);
    }
