...
fn benchmark_tree_sitter_parsing(file_path: &Path) -> Result<()> {
    println!(
        "🔍 Benchmarking Tree-sitter vs Regex for: {}",
        file_path.display()
    );
    println!("==========================================");
    ...
    println!(
        "  - Parser initialization: {:.3}ms",
        tree_sitter_init_time.as_secs_f64() * 1000.0
    );
    ...
    println!(
        "  - Total Tree-sitter time: {:.3}ms",
        total_tree_sitter.as_secs_f64() * 1000.0
    );
    ...
    println!(
        "  - Indexer initialization: {:.3}ms",
        regex_init_time.as_secs_f64() * 1000.0
    );
    ...
    println!(
        "  - Symbol extraction: {:.3}ms",
        regex_extract_time.as_secs_f64() * 1000.0
    );
    println!("  - Symbols found: {}", symbols.len());

    let total_regex = regex_init_time + regex_extract_time;
    println!(
        "  - Total Regex time: {:.3}ms",
        total_regex.as_secs_f64() * 1000.0
    );
    ...
    match symbol_count.cmp(&symbols.len()) {
        std::cmp::Ordering::Greater => {
            println!(
                "  - Tree-sitter found {} more symbols",
                symbol_count - symbols.len()
            );
        }
        std::cmp::Ordering::Less => {
            println!(
                "  - Regex found {} more symbols",
