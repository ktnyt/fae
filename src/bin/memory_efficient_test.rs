...
fn main() -> anyhow::Result<()> {
    ...
    println!("🔍 Testing Memory-Efficient Cache for: {}", directory.display());
    ...
    match old_indexer.load_cache(directory) {
        Ok(stats) => {
            println!("📊 Existing cache: {} files, {} symbols", stats.total_files, stats.total_symbols);
            ...
            for (i, file_name) in test_files.iter().enumerate() {
                ...
                if start_idx < all_symbols.len() {
                    ...
                    println!("💾 Updating cache for {}: {} symbols", file_name, file_symbols.len());
            ...
            println!("  Traditional cache: ~{:.2} MB (estimated)", 
                     all_symbols.len() as f64 * 568.0 / 1024.0 / 1024.0);
            println!("  Memory-efficient cache: {:.2} MB", cache_manager.memory_usage_mb());
            ...
            println!("  Current memory usage: {:.2} MB", cache_manager.memory_usage_mb());
