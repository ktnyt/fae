...
fn main() -> anyhow::Result<()> {
    ...
    println!("🚀 Benchmarking indexing performance for: {}", path.display());
    println!("==========================================");
    ...
    println!("⏱️  Discovered {} files in {:.3}s", files.len(), discovery_time.as_secs_f64());
    ...
    for (i, file_path) in files.iter().enumerate() {
        ...
        if (i + 1) % (files.len() / 10).max(1) == 0 {
            ...
            println!("   Progress: {}% ({}/{} files, {:.1}s elapsed)", 
                     progress, i + 1, files.len(), elapsed.as_secs_f64());
        }
    }
    
    let indexing_time = start.elapsed();
    println!("⏱️  Extracted {} symbols from {} files in {:.3}s", 
             total_symbols, processed_files, indexing_time.as_secs_f64());
    
    // Phase 3: Total time
    let total_time = discovery_time + indexing_time;
    println!("\n📊 Performance Summary:");
    println!("   File discovery: {:.3}s ({:.1}%)", 
             discovery_time.as_secs_f64(), 
             discovery_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
    println!("   Symbol extraction: {:.3}s ({:.1}%)", 
             indexing_time.as_secs_f64(),
             indexing_time.as_secs_f64() / total_time.as_secs_f64() * 100.0);
    println!("   Total time: {:.3}s", total_time.as_secs_f64());
    println!("   Symbols per second: {:.1}", total_symbols as f64 / indexing_time.as_secs_f64());
    println!("   Files per second: {:.1}", processed_files as f64 / indexing_time.as_secs_f64());
    ...
    if !slow_files.is_empty() {
        ...
        for (i, (path, duration, symbols)) in slow_files.iter().take(10).enumerate() {
            println!("   {}. {} - {:.1}s ({} symbols, {:.1} symbols/s)", 
                     i + 1,
        ...
        println!("   Slow files account for {:.1}% of total indexing time", slow_percentage);
    }
