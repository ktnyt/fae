...
fn main() -> anyhow::Result<()> {
    ...
    println!("📊 Initial memory: {} MB", 
             initial_memory.map_or("N/A".to_string(), |m| format!("{:.2}", m as f64 / 1024.0 / 1024.0)));
    ...
    match indexer.load_cache(directory) {
        Ok(stats) => {
            println!("📦 Cache loaded: {} files, {} symbols", stats.total_files, stats.total_symbols);
            ...
            if let (Some(pre), Some(post)) = (pre_cache_memory, post_cache_memory) {
                ...
                println!("📈 Cache memory usage: {:.2} MB", cache_memory as f64 / 1024.0 / 1024.0);
                
                // 1シンボルあたりのメモリ使用量
                if stats.total_symbols > 0 {
                    let bytes_per_symbol = cache_memory / stats.total_symbols as usize;
            ...
            if let (Some(post), Some(final_mem)) = (post_cache_memory, final_memory) {
                ...
                println!("📈 Symbols vector memory: {:.2} MB", symbols_memory as f64 / 1024.0 / 1024.0);
            }
