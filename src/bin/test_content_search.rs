...
async fn main() -> anyhow::Result<()> {
    ...
    indexer.index_directory(std::path::Path::new("."), &patterns).await?;
    let indexing_duration = start_indexing.elapsed();
    
    let symbols = indexer.get_all_symbols();
    println!("📚 Indexed {} symbols in {:?}\n", symbols.len(), indexing_duration);
    ...
    for i in 0..5 {
        ...
        println!("   Run {}: {} results in {:?}", i + 1, results.len(), duration);
    }
