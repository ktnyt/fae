use clap::Parser;
use sfs::{TreeSitterIndexer, FuzzySearcher, types::*, run_tui};
use std::path::Path;

#[derive(Parser)]
#[command(name = "sfs")]
#[command(about = "Symbol Fuzzy Search - Fast code symbol search tool")]
#[command(long_about = "SFS is a high-performance code search tool that finds symbols (functions, classes, variables, etc.) in your codebase using fuzzy matching.")]
#[command(version)]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.query {
        Some(query) => {
            // CLI search mode
            perform_search(query, directory, cli.verbose).await?;
        }
        None => {
            // TUI mode
            if cli.verbose {
            ...
            run_tui(directory.to_path_buf(), cli.verbose, respect_gitignore).await?;
        }
    }
...
async fn perform_search(query: String, directory: &Path, verbose: bool) -> anyhow::Result<()> {
    if verbose {
    ...
    // Index directory with default patterns (all files)
