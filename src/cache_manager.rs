...
impl MemoryEfficientCacheManager {
    ...
    pub fn get_file_symbols(&mut self, file_path: &str) -> Result<Vec<CodeSymbol>> {
        ...
        let symbol_locations = self.index.files.get(file_path)
    ...
    fn get_chunk_symbols(&mut self, chunk_id: ChunkId) -> Result<Vec<CodeSymbol>> {
        ...
        while self.current_memory_usage + symbols_memory > self.max_memory_usage {
            if let Some((_, evicted_symbols)) = self.symbol_cache.pop_lru() {
                self.current_memory_usage -= evicted_symbols.len() * std::mem::size_of::<CodeSymbol>();
            } else {
                break;
            }
        }
    ...
    /// ファイルのキャッシュエントリを更新
    pub fn update_file_cache(&mut self, file_path: &str, hash: String, symbols: Vec<CodeSymbol>) -> Result<()> {
        // シンボルをチャンク分割
        ...
        for (chunk_symbols, chunk_id) in chunks {
            ...
            // チャンク情報を更新
            self.index.chunk_info.insert(chunk_id, ChunkInfo {
    ...
    /// シンボルをチャンクに分割
    fn split_symbols_into_chunks(&self, symbols: Vec<CodeSymbol>) -> Vec<(Vec<CodeSymbol>, ChunkId)> {
