use super::content_backend::ContentSearchBackend;
...
/// High-performance literal search engine using external tools (ripgrep/ag) with fallback
pub struct LiteralSearchEngine {
    symbols: Vec<CodeSymbol>,
}

impl LiteralSearchEngine {
    /// Create new literal search engine with symbol list for fallback
    pub fn new(symbols: Vec<CodeSymbol>) -> Self {
        Self { symbols }
    }

    /// Update the symbols for fallback search
    pub fn update_symbols(&mut self, symbols: Vec<CodeSymbol>) {
        self.symbols = symbols;
    }

    /// Perform literal content search with backend detection
    pub fn search(&self, query: &str, options: &SearchOptions) -> Vec<SearchResult> {
        if query.trim().is_empty() {
            return vec![];
        }

        let backend = ContentSearchBackend::detect();

        match backend {
            ContentSearchBackend::Ripgrep => {
                if let Ok(results) = self.search_with_ripgrep(query, options) {
                    return results;
                }
                // Fall back to ag if ripgrep fails
                if let Ok(results) = self.search_with_ag(query, options) {
                    return results;
                }
                // Final fallback
                self.search_fallback(query, options)
            }
            ...
            ContentSearchBackend::Fallback => self.search_fallback(query, options),
        }
    }

    /// High-performance content search using ripgrep
    fn search_with_ripgrep(&self, query: &str, options: &SearchOptions) -> Result<Vec<SearchResult>> {
    ...
    fn search_with_ag(&self, query: &str, options: &SearchOptions) -> Result<Vec<SearchResult>> {
        ...
        cmd.arg("--line-numbers") // Show line numbers
            ...
            .arg("--max-filesize") // Limit file size
            .arg("1M"); // 1MB limit
    ...
    /// Fallback implementation using internal search
    fn search_fallback(&self, query: &str, options: &SearchOptions) -> Vec<SearchResult> {
            ...
            if let Ok(content) = fs::read_to_string(file_path) {
                ...
                for (line_num, line_content) in line_matches {
                    results.push(SearchResult {
                        ...
                        score: 0.2, // Fallback results have lower quality than ripgrep/ag
                    });
                }
            }
        }
    ...
    /// Get optimized search paths to minimize tool overhead
    fn get_optimized_search_paths(&self) -> Vec<PathBuf> {
