...
use sfs_rs::{indexer::TreeSitterIndexer, searcher::FuzzySearcher, types::*};
use std::path::Path;
use tempfile::TempDir;
use std::fs;

// Helper function to create test files with many symbols
fn create_symbol_heavy_files(dir: &Path, file_count: usize, symbols_per_file: usize) -> anyhow::Result<()> {
    for file_idx in 0..file_count {
        ...
        for i in 0..symbols_per_file {
            content.push_str(&format!(
                ...
                file_idx, i, 
                ["search", "find", "filter", "map", "reduce", "process", "handle", "execute"][i % 8]
            ));
...
fn bench_symbol_type_filtering(c: &mut Criterion) {
    ...
    let filter_options = [
        ...
        ("multiple_types", vec![SymbolType::Function, SymbolType::Class]),
    ];
