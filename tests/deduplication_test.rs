...
mod deduplication_tests {
    ...
    fn create_duplicate_symbol_project(dir: &TempDir) -> anyhow::Result<()> {
        ...
        fs::write(dir_path.join("src/main.ts"), 
            "function main() {}\nconst helper = () => 'main';\nclass App {}")?;
        
        fs::write(dir_path.join("src/components/App.ts"), 
            "function render() {}\nconst helper = () => 'component';\nclass Component {}")?;
        
        fs::write(dir_path.join("src/utils/helper.ts"), 
            "function helper() {}\nconst utility = () => 'util';\nclass Helper {}")?;
        
        fs::write(dir_path.join("tests/main.test.ts"), 
        ...
        fs::write(dir_path.join("tests/index.ts"), "export * from './main.test';")?;
    ...
    async fn should_prioritize_filename_symbols_over_code_symbols() {
        ...
        for (file_path, symbols) in &symbols_by_file {
            let filename_symbols: Vec<_> = symbols.iter()
                .filter(|s| s.symbol_type == SymbolType::Filename)
                .collect();
            
            assert!(!filename_symbols.is_empty(), 
                "File {:?} should have at least one filename symbol", file_path);
            
            // Verify filename symbol has correct name
            if let Some(expected_name) = file_path.file_name() {
                assert!(filename_symbols.iter().any(|s| s.name == expected_name.to_string_lossy()),
                    "File {:?} should have filename symbol with correct name", file_path);
            }
        }
        
        // Verify we have the expected files
        let file_names: Vec<String> = all_symbols.iter()
            .filter(|s| s.symbol_type == SymbolType::Filename)
            .map(|s| s.name.clone())
            .collect();
        ...
        println!("Found {} filename symbols: {:?}", file_names.len(), file_names);
    }
    ...
    async fn should_handle_directory_name_uniqueness() {
        ...
        let dir_symbols: Vec<_> = all_symbols.iter()
        ...
        assert!(dir_name_counts.get("src").map_or(0, |&count| count) > 0, 
            "Should find 'src' directory symbols");
        assert!(dir_name_counts.get("components").map_or(0, |&count| count) > 0, 
            "Should find 'components' directory symbols");
        
        // But they should be associated with correct files
        let src_symbols: Vec<_> = dir_symbols.iter()
            .filter(|s| s.name == "src")
            .collect();
        ...
        for file_name in &src_direct_files {
            assert!(src_symbols.iter().any(|s| 
                s.file.to_string_lossy().contains(file_name)),
                "src directory symbol should be associated with {}", file_name);
        }
        
        // Files in src/components/ and src/utils/ should have "components"/"utils" dirname symbols
        let components_symbols: Vec<_> = dir_symbols.iter()
            .filter(|s| s.name == "components")
            .collect();
        assert!(components_symbols.iter().any(|s| 
            s.file.to_string_lossy().contains("App.ts")),
            "components directory symbol should be associated with App.ts");
            
        let utils_symbols: Vec<_> = dir_symbols.iter()
            .filter(|s| s.name == "utils")
            .collect();
        assert!(utils_symbols.iter().any(|s| 
            s.file.to_string_lossy().contains("helper.ts")),
    ...
    async fn should_preserve_file_representative_selection_logic() -> anyhow::Result<()> {
        ...
        fs::write(dir_path.join("calculator.ts"), 
            "class Calculator {\n\
        ...
        let filename_symbols: Vec<_> = file_symbols.iter()
            .filter(|s| s.symbol_type == SymbolType::Filename)
            .collect();
        assert!(!filename_symbols.is_empty(), "Should have filename symbol");
        
        // Should have directory symbol  
        let dirname_symbols: Vec<_> = file_symbols.iter()
            .filter(|s| s.symbol_type == SymbolType::Dirname)
            .collect();
        assert!(!dirname_symbols.is_empty(), "Should have dirname symbol");
        
        // Should have code symbols
        let code_symbols: Vec<_> = file_symbols.iter()
            .filter(|s| matches!(s.symbol_type, SymbolType::Class | SymbolType::Function | SymbolType::Variable))
            .collect();
        assert!(!code_symbols.is_empty(), "Should have code symbols");
        
        // Verify specific symbols exist
        assert!(file_symbols.iter().any(|s| s.name == "calculator.ts" && s.symbol_type == SymbolType::Filename));
        assert!(file_symbols.iter().any(|s| s.name == "Calculator" && s.symbol_type == SymbolType::Class));
        assert!(file_symbols.iter().any(|s| s.name == "add" && s.symbol_type == SymbolType::Function));
        assert!(file_symbols.iter().any(|s| s.name == "helper" && s.symbol_type == SymbolType::Function));
        assert!(file_symbols.iter().any(|s| s.name == "process" && s.symbol_type == SymbolType::Function));
        
        println!("File {} has {} symbols: {} filename, {} dirname, {} code", 
            file_path.display(), file_symbols.len(), 
            filename_symbols.len(), dirname_symbols.len(), code_symbols.len());
    ...
    async fn should_handle_files_with_only_code_symbols() -> anyhow::Result<()> {
        ...
        fs::write(dir_path.join("logic.ts"), 
        ...
        assert!(file_symbols.iter().any(|s| s.symbol_type == SymbolType::Filename),
            "Should have filename symbol even with code symbols present");
        
        // Should have code symbols as well
        assert!(file_symbols.iter().any(|s| s.symbol_type == SymbolType::Function),
            "Should have function symbols");
        assert!(file_symbols.iter().any(|s| s.symbol_type == SymbolType::Class),
            "Should have class symbols");
        
        // Verify that the filename symbol is the expected representative
        let filename_symbol = file_symbols.iter()
        ...
        println!("File with code symbols has {} total symbols", file_symbols.len());
    ...
    async fn should_maintain_consistent_symbol_ordering() {
        ...
        for run in 0..3 {
            ...
            println!("Run {}: Found {} symbols", run + 1, all_runs_symbols[run].len());
        }
        
        // Verify all runs produced the same number of symbols
        let symbol_counts: Vec<usize> = all_runs_symbols.iter().map(|s| s.len()).collect();
        assert!(symbol_counts.iter().all(|&count| count == symbol_counts[0]),
            "Symbol counts should be consistent: {:?}", symbol_counts);
        
        // Verify symbol names are consistent (though order might vary)
        for run_idx in 1..all_runs_symbols.len() {
            let run0_names: std::collections::HashSet<_> = all_runs_symbols[0].iter()
                .map(|s| (&s.name, &s.symbol_type, &s.file))
                .collect();
            let run_names: std::collections::HashSet<_> = all_runs_symbols[run_idx].iter()
                .map(|s| (&s.name, &s.symbol_type, &s.file))
                .collect();
            
            assert_eq!(run0_names, run_names, 
                "Symbol sets should be identical between runs 0 and {}", run_idx);
        }
        
        println!("Deduplication logic is consistent across {} runs", all_runs_symbols.len());
    }
}