...
mod progressive_indexing_tests {
    ...
    fn create_test_project_with_multiple_files(dir: &TempDir) -> anyhow::Result<()> {
        ...
        fs::write(dir_path.join("src/utils.ts"), 
            "const helper = (x: string) => x.toUpperCase();\nclass Manager { process() {} }")?;
        fs::write(dir_path.join("src/types.ts"), 
            "interface User { id: number; name: string; }\ntype Status = 'active' | 'inactive';")?;
        
        // Create JavaScript files  
        fs::write(dir_path.join("tests/test.js"), 
            "function testHelper() { return true; }\nconst assert = require('assert');")?;
        fs::write(dir_path.join("tests/integration.js"), 
            "describe('Integration', () => { it('works', () => {}); });")?;
        
        // Create Python files
        fs::write(dir_path.join("docs/generator.py"), 
            "def generate_docs():\n    pass\n\nclass DocGenerator:\n    def run(self): pass")?;
        
        // Create some larger files to test progressive behavior
        let large_content = (0..100).map(|i| format!("function func{}() {{ return {}; }}", i, i))
            .collect::<Vec<_>>().join("\n");
    ...
    async fn should_perform_basic_progressive_indexing() {
        ...
        assert!(all_symbols.len() > 10, "Should extract symbols from multiple files");
        
        // Verify symbols from different file types
        assert!(all_symbols.iter().any(|s| s.name == "main"), "Should find main function");
        assert!(all_symbols.iter().any(|s| s.name == "Manager"), "Should find Manager class");
        assert!(all_symbols.iter().any(|s| s.name == "testHelper"), "Should find testHelper function");
        assert!(all_symbols.iter().any(|s| s.name == "generate_docs"), "Should find Python function");
        
        // Verify performance is reasonable
        assert!(duration < Duration::from_secs(5), "Indexing should complete within 5 seconds");
        
        println!("Progressive indexing completed in {:?} with {} symbols", duration, all_symbols.len());
    }
    ...
    fn should_handle_extract_symbols_sync_correctly() {
        ...
        assert!(symbols.iter().any(|s| s.name == "asyncFunc" && s.symbol_type == SymbolType::Function));
        assert!(symbols.iter().any(|s| s.name == "arrow" && s.symbol_type == SymbolType::Function));
        assert!(symbols.iter().any(|s| s.name == "TestClass" && s.symbol_type == SymbolType::Class));
        assert!(symbols.iter().any(|s| s.name == "test.ts" && s.symbol_type == SymbolType::Filename));
        
        assert!(symbols.len() >= 4, "Should extract at least 4 symbols (function, arrow, class, filename)");
    }
    ...
    fn should_maintain_symbol_consistency_across_multiple_runs() {
        ...
        assert!(symbol_counts.iter().all(|&count| count == symbol_counts[0]),
            "Symbol counts should be consistent across runs: {:?}", symbol_counts);
