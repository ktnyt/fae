...
mod platform_compatibility_tests {
    ...
    fn create_cross_platform_project(dir: &TempDir) -> anyhow::Result<()> {
        ...
        fs::write(dir_path.join("src/components/ui/Button.tsx"), r#"
...
"#)?;
        ...
        // Unix系のパーミッション関連コード
        fs::write(dir_path.join("src/utils/platform/fileUtils.ts"), r#"
...
"#)?;
        ...
        // Windows: 特殊文字制限
        fs::write(dir_path.join("windows_compat.ts"), r#"
...
"#)?;
        ...
        fs::write(dir_path.join(long_filename), r#"
...
"#)?;
        ...
        fs::write(dir_path.join("macOS_unicode_test_café.ts"), r#"
...
"#)?;
        ...
        // クロスプラットフォーム設定ファイル
        fs::write(dir_path.join("platform.config.json"), r#"{
    "platforms": {
        "windows": {
            "pathSeparator": "\\",
            "lineEnding": "\r\n",
    ...
    async fn should_handle_platform_specific_path_separators() -> anyhow::Result<()> {
        ...
        assert!(indexing_duration < Duration::from_secs(10), 
            "Should handle cross-platform paths efficiently, took {:?}", indexing_duration);
        assert!(!all_symbols.is_empty(), "Should extract symbols from cross-platform project");
        
        // プラットフォーム固有のシンボル確認
        assert!(all_symbols.iter().any(|s| s.name == "Button"), "Should find Button component");
        assert!(all_symbols.iter().any(|s| s.name == "FileUtils"), "Should find FileUtils class");
        assert!(all_symbols.iter().any(|s| s.name == "WindowsCompatibility"), "Should find Windows compat class");
        assert!(all_symbols.iter().any(|s| s.name == "UnixCompatibility"), "Should find Unix compat class");
        assert!(all_symbols.iter().any(|s| s.name == "MacOSUnicodeHandler"), "Should find macOS Unicode handler");
        
        // パスの正規化確認
        for symbol in &all_symbols {
            let path_str = symbol.file.to_string_lossy();
            // すべてのパスが有効であることを確認
            assert!(symbol.file.exists() || path_str.contains("temp"), 
                "All symbol paths should be valid: {}", path_str);
        }
        
        println!("✅ Platform-specific paths: {} symbols indexed in {:?}", 
    ...
    async fn should_handle_unicode_filenames_across_platforms() -> anyhow::Result<()> {
        ...
        let unicode_symbols: Vec<_> = all_symbols.iter()
            .filter(|s| s.file.to_string_lossy().contains("café") || 
                       s.file.to_string_lossy().contains("unicode"))
            .collect();
        
        assert!(!unicode_symbols.is_empty(), "Should handle Unicode filenames");
        
        // macOS Unicode 正規化関連のシンボル確認
        assert!(all_symbols.iter().any(|s| s.name == "MacOSUnicodeHandler"), 
            "Should find macOS Unicode handler");
        assert!(all_symbols.iter().any(|s| s.name == "normalizeUnicode"), 
            "Should find Unicode normalization methods");
        
        println!("✅ Unicode filenames: {} symbols, {} Unicode files", 
            all_symbols.len(), unicode_symbols.len());
    ...
    async fn should_respect_platform_file_system_limits() -> anyhow::Result<()> {
        ...
        let long_filename_symbols: Vec<_> = all_symbols.iter()
            .filter(|s| s.file.file_name()
                .map(|name| name.to_string_lossy().len() > 100)
                .unwrap_or(false))
            .collect();
        
        assert!(!long_filename_symbols.is_empty(), "Should handle long filenames");
        
        // プラットフォーム制限のシンボル確認
        assert!(all_symbols.iter().any(|s| s.name == "UNIX_MAX_FILENAME_LENGTH"), 
            "Should find Unix limits constants");
        assert!(all_symbols.iter().any(|s| s.name == "checkPathLimits"), 
            "Should find path limit checking functions");
        
        // 性能が適切であることを確認
        assert!(indexing_duration < Duration::from_secs(5), 
            "Should handle file system limits efficiently");
        
        println!("✅ File system limits: {} symbols, {} long filenames in {:?}", 
            all_symbols.len(), long_filename_symbols.len(), indexing_duration);
    ...
    async fn should_handle_case_sensitivity_differences() -> anyhow::Result<()> {
        ...
        for (filename, content) in test_files {
            ...
            if let Err(_) = fs::write(&file_path, content) {
                ...
                println!("⚠️  Skipped creating {} due to platform case sensitivity", filename);
            }
        }
        ...
        assert!(!upper_results.is_empty() || !lower_results.is_empty(), 
            "Should handle case sensitivity search variations");
        
        // プラットフォーム固有のクラスが見つかることを確認
        let case_classes: Vec<_> = all_symbols.iter()
            .filter(|s| s.name.contains("CaseTest"))
            .collect();
        
        assert!(!case_classes.is_empty(), "Should find case test classes");
        
        println!("✅ Case sensitivity: {} symbols, {} case test classes", 
            all_symbols.len(), case_classes.len());
    ...
    async fn should_handle_platform_specific_line_endings() -> anyhow::Result<()> {
        ...
        let unix_content = "export class UnixLineEndings {\n    method() {\n        return 'unix';\n    }\n}";
        ...
        assert!(all_symbols.iter().any(|s| s.name == "UnixLineEndings"), 
            "Should handle Unix line endings");
        assert!(all_symbols.iter().any(|s| s.name == "WindowsLineEndings"), 
            "Should handle Windows line endings");
        assert!(all_symbols.iter().any(|s| s.name == "MixedLineEndings"), 
    ...
    async fn should_work_consistently_across_platforms() -> anyhow::Result<()> {
        ...
        for i in 0..3 {
            ...
            println!("🔄 Run {}: {} symbols in {:?}", i + 1, symbols.len(), duration);
        }
        ...
        for (i, duration) in durations.iter().enumerate() {
            ...
            assert!(variance < 3.0, 
                ...
                i + 1, duration, avg_duration, variance);
        }
        
        println!("✅ Consistency: {} runs with stable {} symbols and avg {:?}", 
            results.len(), first_count, avg_duration);
    ...
    async fn should_handle_concurrent_access_on_current_platform() -> anyhow::Result<()> {
        ...
        for (i, _, duration) in &results {
            assert!(duration < &Duration::from_secs(10), 
                "Concurrent task {} should complete within reasonable time: {:?}", i, duration);
        }
        
        println!("✅ Concurrent access: {} tasks with consistent {} symbols", 
