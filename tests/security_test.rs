...
mod security_tests {
    ...
    fn create_malicious_files_project(dir: &TempDir) -> anyhow::Result<()> {
        ...
        // 正常なファイル
        fs::write(dir_path.join("normal.ts"), "function normal() { return 'safe'; }")?;
        
        // パストラバーサル攻撃を試みるファイル名（無害化されるべき）
        // 注：実際のファイルシステムでは ../../../ などは無効なので別のアプローチ
        fs::create_dir_all(dir_path.join("subdir"))?;
        fs::write(dir_path.join("subdir/..%2e%2f..%2e%2fetc%2epasswd.ts"), "// malicious file")?;
        ...
        // 特殊文字を含むファイル名
        fs::write(dir_path.join("special'\"<>&;$()|file.ts"), "// special chars")?;
        
        // 空のファイル名は作れないので、隠しファイル
        fs::write(dir_path.join(".hidden_file.ts"), "// hidden file")?;
        
        // Unicode文字を含むファイル名
        fs::write(dir_path.join("unicode_日本語_файл_🎉.ts"), "// unicode test")?;
        
        // ファイル名にnull文字を含む試行（ファイルシステムレベルで防がれる）
        // 代わりに制御文字を含むファイル名をテスト
        let control_char_filename = format!("control\x01\x02.ts");
    ...
    fn create_symlink_project(dir: &TempDir) -> anyhow::Result<()> {
        ...
        // 正常なファイル
        fs::write(dir_path.join("target.ts"), "function target() { return 'original'; }")?;
        ...
        #[cfg(unix)]
        {
            ...
            // 相対シンボリックリンク
            if let Err(_) = symlink("target.ts", dir_path.join("symlink.ts")) {
                // シンボリックリンク作成に失敗した場合は通常ファイルで代替
                fs::write(dir_path.join("symlink.ts"), "// symlink fallback")?;
            }
            
            // 循環シンボリックリンクの作成試行
            if let Err(_) = symlink("circular_a.ts", dir_path.join("circular_b.ts")) {
                fs::write(dir_path.join("circular_b.ts"), "// circular fallback")?;
            }
            if let Err(_) = symlink("circular_b.ts", dir_path.join("circular_a.ts")) {
                fs::write(dir_path.join("circular_a.ts"), "// circular fallback")?;
            }
            
            // 存在しないファイルへのシンボリックリンク（dangling link）
            if let Err(_) = symlink("nonexistent.ts", dir_path.join("dangling.ts")) {
        ...
        #[cfg(not(unix))]
        {
            ...
            fs::write(dir_path.join("circular_a.ts"), "// Windows circular fallback a")?;
            fs::write(dir_path.join("circular_b.ts"), "// Windows circular fallback b")?;
    ...
    async fn should_handle_malicious_filenames_safely() -> anyhow::Result<()> {
        ...
        // インデックシングが完了することを確認
        assert!(indexing_duration < Duration::from_secs(30), "Should complete within reasonable time");
        assert!(!all_symbols.is_empty(), "Should extract symbols from valid files");
        
        // 正常なファイルのシンボルが見つかることを確認
        assert!(all_symbols.iter().any(|s| s.name == "normal"), "Should find symbols from normal files");
        
        // ファイル名に関係なく、ファイルの内容からシンボルが抽出されることを確認
        let file_symbols: Vec<_> = all_symbols.iter()
            .filter(|s| s.symbol_type == SymbolType::Filename)
            .collect();
        assert!(!file_symbols.is_empty(), "Should handle various filename formats");
        
        // Unicode文字を含むファイル名も適切に処理されることを確認
        assert!(all_symbols.iter().any(|s| s.name.contains("unicode")), 
            "Should handle Unicode filenames");
        
        println!("✅ Malicious filename test: {} symbols extracted in {:?}", 
    ...
    async fn should_handle_malicious_file_content_safely() -> anyhow::Result<()> {
        ...
        assert!(indexing_duration < Duration::from_secs(60), 
            "Should complete within 60 seconds even with malicious content, took {:?}", indexing_duration);
        ...
        assert!(!all_symbols.is_empty(), "Should extract some symbols without crashing");
        
        // 膨大な数のシンボルも適切に処理されることを確認
        let function_symbols: Vec<_> = all_symbols.iter()
            .filter(|s| s.symbol_type == SymbolType::Function)
            .collect();
        assert!(function_symbols.len() > 100, "Should handle files with many symbols");
        
        println!("✅ Malicious content test: {} symbols extracted in {:?}", 
            all_symbols.len(), indexing_duration);
    ...
    async fn should_handle_symlinks_and_special_files_safely() -> anyhow::Result<()> {
        ...
        assert!(indexing_duration < Duration::from_secs(30), 
            "Should complete within 30 seconds without infinite loops, took {:?}", indexing_duration);
        
        // 元のファイルからはシンボルが抽出されることを確認
        assert!(all_symbols.iter().any(|s| s.name == "target"), "Should find symbols from target files");
        
        // シンボリックリンク関連でクラッシュしないことを確認
        assert!(!all_symbols.is_empty(), "Should extract symbols without crashing on symlinks");
        
        println!("✅ Symlink safety test: {} symbols extracted in {:?}", 
            all_symbols.len(), indexing_duration);
    ...
    async fn should_validate_search_inputs_safely() -> anyhow::Result<()> {
        let temp_dir = TempDir::new().unwrap();
        fs::write(temp_dir.path().join("test.ts"), "function testFunc() { return 'test'; }")?;
        ...
        for query in &malicious_queries {
            ...
            assert!(search_duration < Duration::from_millis(1000), 
                "Search for '{}' should complete within 1 second, took {:?}", 
                query.chars().take(50).collect::<String>(), search_duration);
            ...
            for result in &results {
                assert!(!result.symbol.name.is_empty(), "Symbol names should not be empty");
                assert!(result.symbol.file.exists() || result.symbol.file.to_string_lossy().contains("test.ts"), 
                    "Symbol files should exist or be the test file");
                assert!(result.score.is_finite(), "Scores should be finite numbers");
            }
        }
        
        println!("✅ Search input validation: All {} malicious queries handled safely", 
    ...
    async fn should_enforce_resource_limits() -> anyhow::Result<()> {
        ...
        assert!(files_per_second > 5.0, 
            "Should process at least 5 files per second, got {:.2}", files_per_second);
        
        // メモリ使用量が合理的であることを間接的に確認
        assert!(all_symbols.len() > 100, "Should extract substantial number of symbols");
        assert!(all_symbols.len() < 10_000, "Should not create excessive symbols");
        ...
        assert!(search_duration < Duration::from_millis(100), 
            "Search should be fast even with many files, took {:?}", search_duration);
        assert!(!results.is_empty(), "Should find function symbols");
        
        println!("✅ Resource limits test: {} files processed in {:?}, search took {:?}", 
    ...
    async fn should_handle_filesystem_edge_cases() -> anyhow::Result<()> {
        ...
        // 合理的な時間内で完了することを確認
        assert!(indexing_duration < Duration::from_secs(30), 
            "Should handle filesystem edge cases within 30 seconds, took {:?}", indexing_duration);
        
        // 有効なファイルからはシンボルが抽出されることを確認
        assert!(all_symbols.iter().any(|s| s.name == "readonly"), "Should find readonly files");
        assert!(all_symbols.iter().any(|s| s.name == "deep"), "Should find deeply nested files");
        assert!(all_symbols.iter().any(|s| s.name == "a" || s.name == "a.ts"), "Should find short filename files");
        
        // 0バイトファイルでもクラッシュしないことを確認
        let _empty_file_symbols: Vec<_> = all_symbols.iter()
            .filter(|s| s.file.to_string_lossy().contains("empty.ts"))
            .collect();
        // 0バイトファイルからはシンボルは抽出されないが、ファイル名は抽出される可能性
        
        println!("✅ Filesystem edge cases test: {} symbols extracted in {:?}", 
            all_symbols.len(), indexing_duration);
    ...
    async fn should_handle_concurrent_access_safely() -> anyhow::Result<()> {
        ...
        for (i, _, duration) in &results {
            assert!(duration < &Duration::from_secs(10), 
                "Concurrent indexer {} should complete within 10 seconds, took {:?}", i, duration);
        }
        
        println!("✅ Concurrent access test: {} indexers completed with consistent results", 
            results.len());
