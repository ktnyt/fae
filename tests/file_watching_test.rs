use anyhow::Result;
use sfs::{FileWatcher, TreeSitterIndexer, types::*};
use std::fs;
...
mod file_watching_tests {
    ...
    fn should_handle_invalid_watch_directory() {
        ...
        assert!(watcher.is_err(), "FileWatcher should fail for invalid directory");
    }
...
mod index_update_tests {
    ...
    fn should_apply_add_index_update() -> Result<()> {
        ...
        assert!(final_count > initial_count, "Symbol count should increase after adding file");
        assert!(indexer.is_file_indexed(&test_file), "File should be marked as indexed");
    ...
    fn should_apply_modify_index_update() -> Result<()> {
        ...
        assert!(indexer.is_file_indexed(&test_file), "File should still be indexed after modification");
    ...
    fn should_apply_remove_index_update() -> Result<()> {
        ...
        assert!(indexer.is_file_indexed(&test_file), "File should be indexed after addition");
        ...
        assert!(count_after_remove < count_after_add, "Symbol count should decrease after removal");
        assert!(!indexer.is_file_indexed(&test_file), "File should not be indexed after removal");
    ...
    fn should_handle_reindex_file_operation() -> Result<()> {
        ...
        assert!(symbols.iter().any(|s| s.name == "test_function"), "Should find the test function");
    ...
    fn should_handle_non_existent_file_reindexing() -> Result<()> {
        ...
        match result {
            Ok(symbols) => {
                assert!(symbols.is_empty(), "Non-existent file should return empty symbols");
            }
            Err(_) => {
    ...
    fn should_track_file_and_symbol_counts() -> Result<()> {
        ...
        assert_eq!(final_file_count, initial_file_count + 1, "File count should increase by 1");
        assert_eq!(final_symbol_count, initial_symbol_count + 2, "Symbol count should increase by 2");
...
mod integration_tests {
    ...
    fn should_handle_multiple_file_operations_sequence() -> Result<()> {
        ...
        assert!(indexer.get_symbol_count() >= 3, "Should have at least 3 symbols");
        ...
        assert_eq!(indexer.get_file_count(), 2, "Should have 2 files after removal");
        assert!(!indexer.is_file_indexed(&file3), "Removed file should not be indexed");
        assert!(indexer.is_file_indexed(&file1), "Other files should remain indexed");
        assert!(indexer.is_file_indexed(&file2), "Modified file should remain indexed");
    ...
    fn should_maintain_performance_with_large_symbol_sets() -> Result<()> {
        ...
        assert!(duration.as_millis() < 1000, "Should complete within 1 second for 100 symbols");
        ...
        assert!(update_duration.as_millis() < 500, "Index update should complete within 500ms");
    ...
    fn should_handle_concurrent_file_operations_safely() -> Result<()> {
        ...
        assert_eq!(final_count, 5, "Should have processed all 5 concurrent files");
...
mod error_handling_tests {
    ...
    fn should_handle_invalid_file_content_gracefully() -> Result<()> {
        ...
        match result {
            Ok(symbols) => {
                ...
                assert!(symbols.len() < 10, "Should not extract many symbols from invalid syntax");
            }
            Err(_) => {
        ...
        match result {
            Ok(symbols) => {
                ...
                if !symbols.is_empty() {
                    ...
                    assert!(symbols.iter().all(|s| matches!(s.symbol_type, SymbolType::Filename | SymbolType::Dirname)), 
    ...
    fn should_handle_memory_pressure_gracefully() -> Result<()> {
        ...
        assert!(memory_increase < 100_000_000, "Memory usage should not increase excessively (100MB)");
