...
mod performance_regression_tests {
    ...
    async fn should_maintain_indexing_performance_standards() {
        ...
        assert!(symbols_per_second > 50.0, 
            "Should extract at least 50 symbols per second, got {:.2}", symbols_per_second);
    }
    ...
    async fn should_maintain_search_performance_standards() {
        ...
        assert!(avg_search_time < Duration::from_millis(50), 
            "Average search time should be under 50ms, got {:?}", avg_search_time);
    }
    ...
    fn should_handle_memory_usage_efficiently() {
        ...
        println!("Memory usage: {} bytes for {} symbols", memory_used, symbols.len());
        
        // Should extract many symbols (1000 functions + 1000 variables + filename + dirname)
        assert!(symbols.len() > 1000, "Should extract over 1000 symbols from large file");
        ...
        assert!(memory_used < 10_000_000, 
            "Memory usage should be reasonable: {} bytes", memory_used);
    }
    ...
    async fn should_maintain_consistent_performance_across_runs() {
        ...
        // Run warmup first, then multiple times to check consistency
        {
            ...
            warmup_indexer.index_directory(temp_dir.path(), &patterns).await.unwrap();
        }
        ...
        assert!(max_duration.as_millis() <= min_duration.as_millis() * 5,
            "Performance variance too high: min {:?}, max {:?}", min_duration, max_duration);
        
        let avg_duration = Duration::from_millis(
            (durations.iter().map(|d| d.as_millis()).sum::<u128>() / durations.len() as u128) as u64
        );
        
        println!("Performance consistency: avg {:?}, range {:?} - {:?}", 
    ...
    fn should_handle_regex_pattern_compilation_efficiently() {
        ...
        fs::write(temp_dir.path().join("test.ts"), 
            "function test() {}\nclass Test {}\nconst value = 42;").unwrap();
        ...
        assert!(total_duration < Duration::from_secs(5),
            "100 extractions should complete within 5 seconds, took {:?}", total_duration);
    }
