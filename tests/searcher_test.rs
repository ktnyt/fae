...
mod fuzzy_searcher {
    ...
    mod search_options {
        ...
        fn should_filter_by_multiple_symbol_types() {
            ...
            for result in &results {
                assert!(matches!(
                    ...
                    SymbolType::Function | SymbolType::Class
                ));
            }
        }
        ...
        fn should_exclude_files_when_include_files_is_false() {
            ...
            let has_filename = results
                .iter()
                .any(|r| r.symbol.symbol_type == SymbolType::Filename);
            assert!(!has_filename);
        }
        ...
        fn should_exclude_directories_when_include_dirs_is_false() {
            ...
            let has_dirname = results
                .iter()
                .any(|r| r.symbol.symbol_type == SymbolType::Dirname);
            assert!(!has_dirname);
        }
    ...
    mod result_scoring {
        ...
        fn should_give_exact_matches_better_scores() {
            ...
            if let (Some(exact), Some(partial)) = (exact_match, partial_matches.first()) {
    ...
    mod symbol_updates {
        ...
        fn should_update_symbols_and_search_in_new_set() {
            ...
            let new_symbols = vec![CodeSymbol {
    ...
    mod context_handling {
        ...
        fn should_include_context_in_search_results() {
            ...
            assert_eq!(
                match_result.unwrap().symbol.context,
        ...
        fn should_handle_symbols_without_context() {
            let symbols_without_context = vec![CodeSymbol {
