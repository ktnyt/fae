...
mod error_handling_tests {
    ...
    async fn should_handle_large_files_gracefully() {
        ...
        println!("Large file ({} bytes) processing completed. Symbols found: {}", 
            fs::metadata(&large_file_path).unwrap().len(), symbols.len());
    ...
    fn should_filter_binary_files_correctly() {
        ...
        fs::write(temp_dir.path().join("binary.exe"), b"\x7fELF\x02\x01\x01\x00").unwrap();
        ...
        assert!(!file_filter.should_index_file(&temp_dir.path().join("image.png")),
            "Should not index PNG files");
        assert!(!file_filter.should_index_file(&temp_dir.path().join("binary.exe")),
            "Should not index executable files");
        assert!(!file_filter.should_index_file(&temp_dir.path().join("data.zip")),
            "Should not index ZIP files");
        
        // Test text file acceptance
        assert!(file_filter.should_index_file(&temp_dir.path().join("code.ts")),
    ...
    async fn should_handle_corrupted_files_gracefully() {
        ...
        let invalid_utf8 = vec![0xFF, 0xFE, 0xFD, b'f', b'u', b'n', b'c', b't', b'i', b'o', b'n'];
        ...
        let corrupted_symbols: Vec<_> = symbols.iter()
            .filter(|s| s.file.to_string_lossy().contains("corrupted"))
            .collect();
        
        println!("Corrupted file handling: {} symbols extracted", corrupted_symbols.len());
    }
    ...
    async fn should_handle_permission_denied_files() {
        ...
        fs::write(temp_dir.path().join("readable.ts"), "function readable() {}").unwrap();
        ...
        let result = indexer.index_directory(temp_dir.path(), &patterns).await;
        assert!(result.is_ok(), "Should handle permission denied files gracefully");
        
        let symbols = indexer.get_all_symbols();
        
        // Should still process readable files
        assert!(symbols.iter().any(|s| s.name == "readable"), 
        ...
        println!("Permission handling test completed with {} symbols", symbols.len());
    }
    ...
    async fn should_handle_empty_directories() {
        ...
        let symbols = indexer.get_all_symbols();
        assert!(symbols.is_empty(), "Should have no symbols from empty directories");
    ...
    async fn should_handle_non_existent_directory() {
        ...
        match result {
            Ok(_) => {
                let symbols = indexer.get_all_symbols();
                assert!(symbols.is_empty(), "Should have no symbols from non-existent directory");
    ...
    fn should_handle_deeply_nested_directories() {
        ...
        assert!(processed_files > 0, "Should process files in deeply nested directories");
    ...
    async fn should_handle_files_with_unusual_extensions() {
        ...
        fs::write(temp_dir.path().join("config.ts.bak"), "function backup() {}").unwrap();
        ...
        assert!(!symbols.iter().any(|s| s.file.to_string_lossy().contains(".ts.bak")),
            "Should not match .ts.bak files with .ts pattern");
        assert!(!symbols.iter().any(|s| s.file.to_string_lossy().contains(".ts.old")),
        ...
        assert!(all_symbols.iter().any(|s| s.name.contains(".ts.bak") || s.name.contains(".ts.old") || s.name.contains("no_extension")),
            "Should process files with unusual extensions when using broad pattern");
        
        println!("Unusual extensions handling: {} symbols with broad pattern", all_symbols.len());
    }
    ...
    async fn should_handle_circular_symlinks_gracefully() {
        ...
        let result = indexer.index_directory(temp_dir.path(), &patterns).await;
        assert!(result.is_ok(), "Should handle directory traversal issues gracefully");
        
        let symbols = indexer.get_all_symbols();
        assert!(symbols.iter().any(|s| s.name == "regular"), 
            "Should process regular files even with potential directory issues");
        
        println!("Directory traversal resilience: {} symbols processed", symbols.len());
    }
    ...
    fn should_validate_input_parameters() {
        ...
        assert!(file_filter.matches_patterns(ts_file, &patterns), 
            "Should match TypeScript file pattern");
        
        let txt_file = std::path::Path::new("test.txt");
        assert!(!file_filter.matches_patterns(txt_file, &patterns), 
