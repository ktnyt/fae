...
mod gitignore_functionality {
    ...
    fn create_test_project(dir: &TempDir) -> anyhow::Result<()> {
        ...
        fs::write(
            dir_path.join("main.ts"),
            "function main() {}\nexport { main };",
        )?;
        fs::write(
            dir_path.join("utils.js"),
        ...
        fs::write(
            dir_path.join("secret.ts"),
            "const SECRET = 'password';\nexport { SECRET };",
        )?;
        fs::write(
            dir_path.join("config.local.js"),
        ...
        fs::write(
            dir_path.join("node_modules/package.json"),
            "{\"name\": \"test\"}",
        )?;

        fs::create_dir_all(dir_path.join(".vscode"))?;
        fs::write(
            dir_path.join(".vscode/settings.json"),
            "{\"editor.tabSize\": 2}",
        )?;
    ...
    async fn should_count_symbols_correctly_with_gitignore() {
        ...
        assert!(
            ...
            "Should have found some ignored files when gitignore is disabled"
        );
    }
}
