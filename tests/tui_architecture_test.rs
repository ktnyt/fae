use sfs::{TuiState, TuiSimulator, BackendEvent};
...
fn test_tui_state_basic_functionality() {
    ...
    assert_eq!(state.is_indexing, false);
...
fn test_tui_simulator_creation() {
    ...
    assert_eq!(simulator.is_indexing(), false);
    assert_eq!(simulator.should_quit(), false);
}
...
fn test_basic_indexing_workflow() {
    ...
    // Create a test file
    create_test_file(&temp_dir, "test.ts", r#"
        class TestClass {
            constructor() {}
