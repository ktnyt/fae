...
│   ├── index_manager.rs           # ファイル発見・監視・Git統合
│   ├── cache_manager.rs           # スマートキャッシュ・LRU・永続化
│   └── searchers/
│       ├── mod.rs                 # 検索エンジン統合
│       ├── content_search.rs      # コンテンツ検索
│       ├── symbol_search.rs       # シンボル検索（Tree-sitter）
│       ├── file_search.rs         # ファイル名検索
│       └── regex_search.rs        # 正規表現検索
└── tests/
    ├── integration_tests.rs
    ├── app_test.rs
    ├── search_coordinator_test.rs
    ├── cache_manager_test.rs
    └── fixtures/                  # テスト用ファイル
```
