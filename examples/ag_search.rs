//! Example demonstrating AgActor usage
//!
//! This example shows how to use the AgActor to perform
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔍 Starting AgActor search example");

    // Create channels for actor communication
    let (tx, mut rx) = mpsc::unbounded_channel();
    let (_actor_tx, actor_rx) = mpsc::unbounded_channel();

    // Create the AgActor with regex mode for function pattern matching
    let actor = AgActor::create(actor_rx, tx, SearchMode::Regexp);

    println!("✨ AgActor created successfully with Regexp mode");
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            println!("💡 Make sure 'ag' (The Silver Searcher) is installed and available in PATH");
        }
    }
